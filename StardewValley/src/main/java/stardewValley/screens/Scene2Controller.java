...
public class Scene2Controller extends SceneBase {
    ...
    public void initialize() {
        ...
        background = new Image(getClass().getResourceAsStream("/img/background/scene2Background.png"));
        ...
        this.foxy.setScene(2);

        canvas.heightProperty().addListener((obs, oldVal, newVal) -> {
            foxy.getPosition().setX(940);
            foxy.getPosition().setY(680);
            redraw(); // Redibujamos después de ajustar la posición
        });
    ...
    private void startAnimationThread() {
        ...
        Thread animationThread = new Thread(() -> {
            while (running) {
                Platform.runLater( () -> {
                    redraw();
                    //System.out.println("Position X: " + foxy.getPosition().getX() + "\nPosition Y: " + foxy.getPosition().getY());
                });
                try {
                    Thread.sleep(80);
                } catch (InterruptedException e) {
